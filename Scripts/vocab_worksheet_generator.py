...
By default the look/feel matches your most recent Section 6 worksheet:
  Title: "Avery's WordlyWise - Section 6"
...
def draw_header_page(c, title, subtitle, page, total_pages):
    """Draws the centered title + subtitle; returns next y."""
    y = PAGE_H - M_TOP
    c.setFont(TITLE_FONT, TITLE_SIZE)
    # t = title if not continued else f"{title} (page {page} of {total_pages})"
    t = f"{title} (page {page} of {total_pages})"
    c.drawString((PAGE_W - stringWidth(t, TITLE_FONT, TITLE_SIZE)) / 2, y, t)
    y -= (TITLE_SIZE + 6)
...
# def build_section(c, section_data, title, subtitle):
    ...
    for e in shuffled_entries:
        ...
        sentence = normalize_ascii(e["sentence"])
