...
- **Identity**: `auth.py` blueprint; users stored in SQLite at `{VOCAB_HUNTERS_DB_PATH}/vocabhunters.sqlite3` (stdlib `sqlite3`, WAL mode, `PRAGMA user_version` migrations). Passwords hashed with Werkzeug PBKDF2-SHA256 (600k iterations; scrypt unavailable on some Python 3.9 builds).
...
- **Per-user storage** (`Libraries/user_data.py`): each user gets `{db}/content/users/{user_id}/` with `user_themes/`, `source_datasets/`, and `responses_datastore/` mirroring the global layout. User-owned items surface in the UI/API with a `u--` key prefix so they can never collide with global key_names. `user_id` always comes from the session, never from request input.
...
Set `VOCAB_HUNTERS_DB_PATH` environment variable to point to the homework hero database directory. Expected layout:
- `vocabhunters.sqlite3` - SQLite identity database, created on first startup (see `get_sqlite_db_path()` in `Libraries/reference_data.py`)
- `content/` - All filesystem-based content, rooted via `get_content_dir()`:
...
Phase5's PDF layout uses tightly coupled pixel math — font sizes, margins, word bank height, and per-question line heights all affect vertical flow across pages. Changing any layout constant can shift content onto the wrong page or clip elements. Question pages and the answer key each paginate dynamically to however many pages the vocabulary list needs (see `paginate_blocks()`); after any layout change, test with a real PDF using a small (fits on one question page, one answer-key page), a medium (spans 2+ question pages), and a large (spans 2+ answer-key pages) vocabulary list, and inspect every page.
